...
public class SPPActivity extends AppCompatActivity {
    ...
    private AppCompatSpinner spinnerSppFile;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        spinnerSppFile = findViewById(R.id.sppsetting_recordSpp);

        sharedPreferences = getSharedPreferences(Constants.SPP_SETTING, 0);

        reloadSettingText();

        spinnerSppFile.setAdapter(
                new ArrayAdapter<>(
                        this,
        ...
        spinnerSppFile.setSelection(sharedPreferences.getInt(Constants.KEY_SPP_FILE, Constants.SPP_FILE), true);

        ((TextView) spinnerSppFile.getSelectedView()).setTextColor(getColor(R.color.colorOrange));
        spinnerSppFile.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ((TextView) spinnerSppFile.getSelectedView()).setTextColor(getColor(R.color.colorOrange));
                sharedPreferences.edit().putInt(Constants.KEY_SPP_FILE, position).apply();
            }
        ...
        textViewBtnSppModel.setOnClickListener(v -> {
            ...
            new MaterialDialog.Builder(v.getContext())
                    ...
                    .itemsCallback((dialog, view, which, text) -> {
                        sharedPreferences.edit().putInt(Constants.KEY_SPP_MODEL, which).apply();

                        textViewSppModel.setText(text.toString());
                    })
                    .show();
        });
