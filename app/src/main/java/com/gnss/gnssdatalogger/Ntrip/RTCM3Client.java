...
public class RTCM3Client implements Runnable {
    ...
    private RTCM3ClientListener mRTCM3ClientListener;


    public RTCM3Client(String host, int port, String mountpoint, String username, String password, RTCM3ClientListener RTCM3ClientListener) {
        this.host = host;
        this.port = port;
        this.mountpoint = mountpoint;
        this.username = username;
        this.password = password;
        this.mRTCM3ClientListener = RTCM3ClientListener;
    }
    ...
    public void run() {
        try {
            ...
            socket = new Socket();
            socket.connect(socketAddress, 10 * 1000); // 10 second connection timeout
            ...
            if (socket.isConnected()) {
                ...
                requestmsg += "User-Agent: NTRIP JavaNTRIPClient/20200506\r\n";
                requestmsg += "Accept: */*\r\n";
                ...
                System.out.println("Waiting for inital data...");
...
//                                }
    ...
    public static int rtk_crc24q(byte[] buff, int len) {
        int crc = 0;
        int i;
        for (i = 0; i < len; i++) {
            crc = tbl_CRC24Q[(crc >> 16) ^ buff[i] & 0xFF] ^ (crc << 8);
        }
        return crc;
    }

    public static int[] tbl_CRC24Q = {
